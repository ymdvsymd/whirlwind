...
name: log-audit
description: >
  tornado実行ログを分析し、バグ・パフォーマンスボトルネックを検出してbdチケットを起票する。
  最新の.runsログを自動検出、またはパス指定可。タイムライン構築→異常検出→レポート→チケット起票。
  "log-audit", "ログ分析", "バグ検出", "ログレビュー", "run分析", "audit log", "analyze log".
arguments: log_path:tornado.logのパス（省略時は.runs/内の最新ログを自動検出）
---

# log-audit: tornado 実行ログ分析 → バグ検出 → チケット起票

## 引数

```
/log-audit [<path-to-tornado.log>] [--priority=<0-4>] [--dry-run]
```

- `<path-to-tornado.log>`: 分析対象のログファイルパス。省略時は `.runs/` 内の最新 `tornado.log` を自動検出
...
2. `LOG_PATH` が未指定の場合、最新ログを自動検出:
   ...
   ```bash
   ls -dt .runs/*/tornado.log 2>/dev/null | head -1
   ```

   ログが見つからない場合はエラーメッセージを出して終了。

3. ログファイルを読み込み、行数を確認。ログが大きい場合は分割して読む。

---

## Phase 2: タイムライン構築

ログを解析し、以下の構造化データを構築する:
...
1. `DRY_RUN=true` の場合、レポートを出力して終了。

2. ユーザーに検出事項一覧を提示し、チケット化する項目の確認を求める:
   - 各項目の推奨優先度を表示
   - ユーザーが優先度を変更可能
   - ユーザーが除外する項目を選択可能

3. 確認後、`bd create` で一括起票:

   ```bash
   bd create --title="<タイトル>" --description="<詳細>" --type=bug --priority=<N>
   ```
