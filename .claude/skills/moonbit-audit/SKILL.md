...
既存チケットの title と対象ファイルを取得し、`CONFIRMED_FINDINGS` の対象ファイルと照合する。
20 件超の場合は `bd search <keyword>` でモジュール名・ファイル名をキーに候補を絞る。

### 5-2. 重複・依存の判定

- 同一ファイルへの既存チケットがある → 重複候補としてユーザーに確認
- 同一モジュール内の既存チケットがある → 依存候補として記録

### 5-3. Intra-batch 依存の分析

Phase 6 で作成するチケット群について:
- 同一ファイルを対象とするチケット同士 → ファイル依存ヒューリスティクスで方向決定
- 同一モジュール内の型定義変更 + 実装変更 → 型定義が先

分析結果を `DEPENDENCY_PLAN` に格納し、Phase 6 の起票後に依存設定する。
...
既存チケットとの依存も同様に設定する。依存関係がある場合、notes に依存先チケット ID と理由を記載する。
...
### 起票チケット
| ID | タイトル | タイプ | 優先度 | カテゴリ | 依存 |
|----|---------|--------|--------|---------|------|
| xxx-001 | ... | task | P3 | コーディングスタイル | <- xxx-002 |
...
- **bd-detail** — チケット精緻化時の依存関係分析。moonbit-audit で起票後に依存を見直す場合に使用する
